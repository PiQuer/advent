...
round_1 = dataset_parametrization(year="2023", day="11", examples=[("", 374)], result=9445168, dataset_class=DataSet,
                                  expansion=2)
round_2 = dataset_parametrization(year="2023", day="11", examples=[], result=742305960572, dataset_class=DataSet,
                                  expansion=1000000)
pytest_generate_tests = generate_rounds(round_1, round_2)

def test_day_11(dataset: DataSet):
    assert sum(np.sum(np.abs(a-b))
               for a, b in combinations(dataset.galaxies(dataset.params["expansion"]), 2)) == dataset.result
