...
def steps(h_idx: int, s_idx: int) -> int:
    return abs(2 * (h_idx - s_idx) - 3) - int(h_idx in (0, 6))
...
def next_states_moving_in(state: State, seen: dict[State], h_idx: Optional[int] = None,
    ...
    for h_idx_2, a in loop:
        s_idx = a - 1
        if a and not any(state.side_rooms[s_idx]) and free_hallway(state, h_idx_2, s_idx):
            new_energy = state.energy + (steps(h_idx_2, s_idx) + len(state.side_rooms[s_idx])) * 10**s_idx
            # noinspection PyTypeChecker
            new_state = State(
                hallway=state.hallway[:h_idx_2] + (0,) + state.hallway[h_idx_2+1:],
